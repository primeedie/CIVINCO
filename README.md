...
Every source and study entry has an examination and a numbered Set. Filters apply across the requested navigation order: **Home → Study Guide → Concepts & Formulas → Practice → Flashcards → Progress**. Change a file’s category from its details. Existing questions and historical attempts retain the category they were originally studied under.

## Study workflow

1. **Home:** upload multiple files and choose automatic categorization or a manual SPEX and Set. Automatic categorization reads each file’s first page during its first extraction request, so it does not add an API call. PDF, PNG, JPG, UTF-8 TXT, and MD are supported; up to 20 files per upload, each up to 50 MB.
...
4. **Study Guide:** choose a SPEX and Set to read topic-first lessons assembled locally from extracted concepts, reviewed general equations, applicability conditions, source links, and practice results. Unreviewed, uncertain, and case-specific calculations are kept out of the memory list. Opening the guide does not call Gemini.
5. **Practice:** choose **Generate new questions**, one or more Sets, a total question count, and difficulty. AI questions use up to 30 selected source entries; new generations rotate the selection. Formula sources must be approved and have valid notation. Gemini returns validated drawing primitives for problems that need a force, structure, survey, hydraulic, soil, or geometry diagram. Generated questions and diagrams receive an independent solution check before saving. Enter a numeric answer in the displayed unit; answers are checked on the server and initial state never includes answer keys or solutions.
6. **Flashcards:** study concepts, equations, or a mixed deck. Equations and variable definitions render with KaTeX. Unapproved or uncertain formulas are visibly marked for source review. Rate your recall to schedule reviews: Again = 10 minutes, Hard = 1 day, Good = an expanding interval, Easy = a longer interval. Use due-only filtering or browse the whole deck.
7. **Progress:** view first-attempt accuracy, recent activity, flashcard reviews, and topic strengths/weaknesses. Repeated attempts do not inflate accuracy. Viewing the solution before answering is tracked separately and excluded from accuracy. Topic labels start after three scored attempts.
