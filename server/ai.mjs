...
Use valid KaTeX LaTeX without dollar delimiters. Preserve the exact symbols: Greek letters, uppercase/lowercase, summation limits, subscripts, superscripts, primes, vectors, fractions, and units. Never replace a symbol with a lookalike. Give each formula a short descriptive title, a topic, definitions and units of variables, and applicability conditions supported by the source. Set equationScope to "general" for a reusable governing equation and "case-specific" for a substitution, component equation, worked-example relationship, or numerical result whose coefficients or totals come from this page's particular geometry and loads. If a variable definition is absent, say "Not defined on this page"; do not invent it. Flag unclear notation as uncertain and explain it in note. Distinguish physical symbol N (normal force) from unit N (newton).
