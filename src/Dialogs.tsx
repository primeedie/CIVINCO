...
export function SourceDialog({ doc, state, refresh, onClose, onLibrary }: { doc: Source; state: State; refresh: () => Promise<void>; onClose: () => void; onLibrary: () => void }) {
  ...
  return <Modal title="Source details" onClose={onClose} wide><div className="source-heading"><span className="file-icon"><FileText size={27} /></span><div><h3>{doc.name}</h3>{doc.autoCategorize && !doc.categoryDetected ? <span className="badge">AUTO · DETECTS FROM PAGE 1</span> : <Badge spex={doc.spex} set={doc.set} />}{doc.categoryReason && <small className="category-reason">{doc.categoryReason}</small>}</div></div>
    {doc.sample ? <Status>These are included examples, not extracted from your course files. Upload your own materials to build a course-specific library.</Status> : <>
      <div className="source-stats"><div><strong>{done} / {doc.totalPages}</strong><span>{['.txt', '.md'].includes(doc.extension || '') ? 'Text sections' : 'Pages'} extracted</span></div><div><strong>{items.filter(i => i.kind === 'formula').length}</strong><span>Equations found</span></div><div><strong>{pages.filter(p => p.reviewed).length}</strong><span>Pages reviewed by you</span></div></div>
      <div className="progress-track"><span style={{ width: `${done / doc.totalPages * 100}%` }} /></div>
      <p className="fine-print">Gemini processes one page per request. Verify symbols before approval.</p>
      <div className="button-row"><a className="button secondary" href={`/api/documents/${doc.id}/source`} target="_blank" rel="noreferrer"><ExternalLink size={16} /> Open original</a>{working ? <button className="button secondary" disabled={busy} onClick={() => run(() => api(`/documents/${doc.id}/pause`, {}))}><Pause size={16} /> Pause after this page</button> : done < doc.totalPages && <button className="button primary" disabled={busy} onClick={() => run(() => api(`/documents/${doc.id}/extract`, {}))}><Sparkles size={16} />{doc.status === 'stored' ? 'Extract with Gemini' : 'Resume unfinished pages'}</button>}</div>
      ...
      <div className="page-inventory"><h3>Page inventory</h3>{pages.map(page => {
        const formulas = items.filter(i => i.page === page.page && i.kind === 'formula');
        return <div className="page-inventory-row" key={page.id}><div><strong>{['.txt', '.md'].includes(doc.extension || '') ? 'Section' : 'Page'} {page.page}</strong><span>{formulas.length} equations · {page.status}{page.reviewed ? ' · Reviewed' : ''}</span>{page.error && <small className="error-text">{page.error}</small>}{page.warnings.map((w, i) => <small className="error-text" key={i}>{w}</small>)}</div><div className="page-actions"><a href={`/api/documents/${doc.id}/source#page=${page.page}`} target="_blank" rel="noreferrer" aria-label={`Open original page ${page.page}`}><ExternalLink size={15} /></a>{page.status === 'extracted' && <button className="button tiny secondary" disabled={busy || page.reviewed} onClick={() => run(() => api(`/documents/${doc.id}/pages/${page.page}/review`, {}))}>{page.reviewed ? <Check size={14} /> : <ShieldCheck size={14} />}{page.reviewed ? 'Reviewed' : 'Mark reviewed'}</button>}</div></div>;
      })}</div>
    </>}
    <div className="source-bottom"><button className="button secondary" onClick={onLibrary}>Review concepts & formulas <ExternalLink size={15} /></button><div className="form-grid"><label>Examination<select value={spex} onChange={e => setSpex(e.target.value as Spex)}>{(Object.keys(SPEX) as Spex[]).map(s => <option key={s} value={s}>SPEX {s} · {SPEX[s].code}</option>)}</select></label><label>Set<input type="number" min={1} max={999} value={set} onChange={e => setSet(Number(e.target.value))} /></label><button className="button secondary align-end" disabled={busy || working || (spex === doc.spex && set === doc.set)} onClick={() => run(() => api(`/documents/${doc.id}`, { spex, set }, 'PATCH'))}>Save category</button></div></div>
    {error && <Status>{error}</Status>}
    {confirmDelete ? <div className="delete-confirm"><p>Remove this file and its concepts, formulas, questions, and associated progress? This cannot be undone.</p><button className="button danger" disabled={busy} onClick={() => run(async () => { await api(`/documents/${doc.id}`, undefined, 'DELETE'); onClose(); })}>Remove permanently</button><button className="button secondary" onClick={() => setConfirmDelete(false)}>Keep file</button></div> : <button className="text-button danger-text" disabled={working || busy} onClick={() => setConfirmDelete(true)}><Trash2 size={14} /> Remove from library</button>}
  </Modal>;
}
...
export function SettingsDialog({ settings, onClose, refresh }: { settings: State['settings']; onClose: () => void; refresh: () => Promise<void> }) {
  ...
  async function save(test = false) {
    ...
    try {
      ...
      setMessage(test ? 'Connection verified. Your AI study tools are ready.' : 'Settings saved for this server session.');
    } catch (e) { setError((e as Error).message); } finally { setBusy(false); }
  }
  return <Modal title="Gemini settings" onClose={onClose}><div className="settings-illustration"><Sparkles size={29} /></div><p className="modal-intro">Gemini powers classification, extraction, practice questions, solutions, and diagrams.</p><div className="settings-status"><span className={`connection-dot ${settings.connected ? 'connected' : ''}`} />{settings.connected ? 'Gemini connected' : 'No Gemini key connected'}</div>
    ...
    <p className="fine-print"><KeyRound size={15} /> Keys entered here stay in server memory for this session. For persistent setup, use GEMINI_API_KEY and GEMINI_MODEL in the project’s .env file.</p>
    <Status>Extraction uses one Gemini request per page. First-page classification is included in that request.</Status>
    {message && <Status success>{message}</Status>}{error && <Status>{error}</Status>}<div className="modal-actions"><button className="button secondary" disabled={busy || !model.trim()} onClick={() => save()}>Save settings</button><button className="button primary" disabled={busy || !model.trim() || (!key && !settings.connected)} onClick={() => save(true)}>{busy ? <LoaderCircle className="spin" size={16} /> : <CheckCircle2 size={16} />}Save & test connection</button></div>
  </Modal>;
}
